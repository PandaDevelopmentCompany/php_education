...
foreach ($anything_array as $variable_items1_array) {
  foreach ($variable_items1_array as $variable_items2_array){
    echo "$variable_items_2_array" . "<br>";
  }
}
// При процессе прогонки элементов многомерного массива через цикл foreach, та переменная из первого цикла принимает в себя каждый элемент массива, который делится и уже в свою очередь является массивом с набором элементов из дочернего массива.
...
// У массивов и переменных одно пространство имен переменных.
// Поэтому иногда возникает протребность спросить не присвоено ли это имя массиву?

// Функция is_array()
echo (is_array($paper)) ? "Это массив" : "Это не массив";


echo "<br>";
// Для подсчета всех элементов в массиве верхнего уровня используется функция count
echo count($paper);
echo "<br>";
// Если есть необходимость посчитать абсолютно все элементы в многомерном массиве, то тогда добавить второй параметр
echo count($products, 1);
echo "<br>";
// Если стоит единица, то подход является рекурсивным, если ничего или нуль, то только верхний уровень считает
echo "<br>";


// Сортировка. Поддерживает несколько флагов. Работает непосредственно с предоставленным массивом, а не возвращает новый массив с отсортированными элементами. При успешном выполнении сортировки возвращает true, а при возникновении ошибки false. rsort - в обратном порядке
sort($products, SORT_NUMERIC);
sort($products, SORT_STRING);


// Сортировка в случайном порядке через функцию shuffle. Работает непосредственно с предоставленным массивом. В случае успешного завершения работы возвращает true, а в случае ошибки - false
shuffle($products);

echo "<br>";
// Взять строку, и, разделив ее на элементы, поместить каждый элемент в массив. в этой строке элементы перед разделением должны быть отделены одним симоволом (необяхательно пробелом) или строкой символов. Этим символом мы даем понять функции каким разделителем обладает строка, которую надо разделить на элемнеты и поместить в массив. функция explode
$temp_for_explode_function = explode(' ', "Это предложение состоит из 6 слов");
print_r($temp_for_explode_function);
echo "<br>"; 
echo $temp_for_explode_function[2];

echo "<br>";

$temp_for_explode_function_2 = explode('***', "Это***предложение***со***звуздочками");
print_r($temp_for_explode_function_2);

echo "<br>";



// Иногда есть необходимость превратить пары ключ - значению из массива в переменные. extract()
extract($_GET);
// Например, если параметр строки запроса q отправлен сценарию php на ряду со связанным с ним значением hi there, будет создана новая переменная $q, которой будет присвоено это значение.
// Если такая переменная уже существует, то существующая переменная будет переписана на новую. Поэтому необходимо быть осторожным. Или же вооспользоваться добавлением префикса.
extract($_GET, EXTR_PREFIX_ALL, "fromget");
// в этом случае все имена новых переменных будут начинаться с fromget_  (fromget_q).
// Лучше всегда таким образом обрабатывать методы _GET и _POST

// Один из таких случаев – обработка переменных _GET или _POST, отправленных формой сценарию php


echo "<br>";

// Функция compact - противоположная функция к extract. compact создает массив из переменных и их значений.
$fname = "alex";
$sname = "rotman";
$planet = "Earth";
$system = "Sun";
...
print_r($contact);
// В кавычках указываются имена переменных, а не значения и указываются без знака $
echo "<br>";


// Когда идет перебор элементов массива с помощью конструкции foreach..as.. или функции each, они перемещают указатель, который указывает какой элемент необходимо извлечь в следующий раз. И если программе необходимо вернуться к началу массива, то можно воспользоваться функцией reset и она к тому же вернет значение элемента, на котором остановился указатель.
reset($product);
$first_item = reset($product);
// Отбрасывание возвращаемого значения и сохранение первого элемента массива в переменную
end($product);
$end_item = end($product);
// По тойже аналогии работает функция end только наоборот
...
// Функция printf – управляет форматом выводимых данных, через вставку специальных форматирующих символов в строку. Printf ожидает, что для каждого форматирующего символа будет предоставлен аругмент, который будет отображаться с использование заданного формата.
printf("В вашей корзине находится %d покупки", 3);
echo "<br>";
// % – спецификатор

// Пример для отображения аргумента, имеющего значение 123
// %% – Отображение символа % (аргументы не требуются) = %
// b – Отображение аргумента в виде двоичного целого числа = 1111011
// c – Отображение ASCII-символа с кодом, содержащимся в аргумента = {
// d – Отображение аргумента в виде целого десятичного числа со знаком = 123
// e – Отображение аргумента с использование научной формы записи = 1.2300e+2
// f – Отображение аргумента в виде целого числа с плаввающей точкой = 123.000000
...
// X – Отображение аргумента в виде шестнадцатеричного числа с символами в верхнем регистре = 7B


// При составлении большого количества спецификаторов, важно, чтобы их количество совпадало с аргументами.

printf("Меня зовут %s. Мне %d лет, то есть %X в шестнадцатеричном представлении", 'Александр', 29, 29);
echo "<br>";
// Перевод из rgb в hex
printf("<span style='color:#%X%X%X'>Элемент сайта</span>", 65+100, 127, 245);
// в аргументах можно использовать выражения

echo "<br>";
// printf настройка представления данных
// если есть необходимость ограничить дробь до двух знков после плавающей точки, то можно перед f поставить .2
